...
class Crime():
    ...
    def process(self):
        ...
        print(police_df)
        print(f'경찰서DF 칼럼: {police_df.columns}')
        '''
        ...
        for i in arrest_rate_columns:
            police_df.loc[police_df[i] > 100, 1] = 100 #데이터값 기간이 1년을 넘긴 경우가 있어서 100를 max로 지정
        vals = ['살인', '강도', '강간', '절도', '폭력']
        ...
        for i in crime_df.index:
            folium.CircleMarker([crime_df['lats'][i], crime_df['lngs'][i]],
                                radius=crime_df['검거'][i] * 10,
                                fill_color='#0a0a32').add_to(m)

        m.save('admin/crime/data/fmap.html')
    ...
    def crime_police(self, crime_df, reader, vo):
        ...
        crime_df['lats'] = station_lats
        crime_df['lngs'] = station_lngs
        gu_names = []
        ...
        print(dt)
        with open('admin/crime/data/test3.csv', 'w', encoding='UTF-8') as f:
            w = csv.writer(f)
            w.writerow(dt.keys())
