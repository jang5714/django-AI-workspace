...
class NaverMoive(object):
    ...
    def web_scraping(self):
        ...
        driver.get('http://movie.naver.com/movie/sdb/rank/rmovie.naver')
    ...
    def naver_process(self):
        ...
        counts = defaultdict(lambda : [0,0]) #dictionary 를 만드는 팩토리 메소드
...
class Imdb(object):
    ...
    def decode_riview(self, text, reverse_word_index):
    ...
    def imdb_process(self):
        ...
        word_index["<UNK>"] = 2
        word_index["<UNUSED>"] = 3
        # reverse_word_index = dict([(v, k) for (v, k) in word_index.items()])
        # temp = self.decode_riview(train_X[0], reverse_word_index)
